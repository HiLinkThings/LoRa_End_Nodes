function decodeUplink(input) {
  ...
  switch (input.fPort) {
    ...
    case 10:
      ...
      {
        data.temprature_sht30 = (((input.bytes[2] & 0x80 ? input.bytes[2] - 0x100 : input.bytes[2]) << 8) + input.bytes[3]) / 100;
    ...
    case 11:
      ...
      {
        data.temprature_ds18b20 = (((input.bytes[5] & 0x80 ? input.bytes[5] - 0x100 : input.bytes[5]) << 8) + input.bytes[6]) / 100;
      }
      else
        data.uplink_type = "unkonw payload";
